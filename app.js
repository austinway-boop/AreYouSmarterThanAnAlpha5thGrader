(() => {
  ...
  function runChalkAnimation() {
    chalkDone = false;
    const lines = [
      { el: "#chalk-line-1", text: "Are you smarter than a" },
      { el: "#chalk-line-2", text: "5th Grader" },
      { el: "#chalk-line-3", text: "@ Alpha?" },
    ];
  ...
  function finishChalk() {
    ...
    lines.forEach(({ el, text }) => {
      const existing = $(el).textContent.replace(/\u00A0/g, " ").trim();
      if (existing.length < text.length) {
        $(el).innerHTML = "";
        [...text].forEach((char) => {
          const span = document.createElement("span");
          span.className = char === " " ? "chalk-char space" : "chalk-char";
    ...
    setTimeout(() => {
      ...
      $("#start-gate").classList.add("fade-up");
      $("#x-handle").focus();
    }, 200);
